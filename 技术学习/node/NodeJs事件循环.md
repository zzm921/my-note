...
**关联笔记：** \[\[Promise详解]] | \[\[深入浅出的nodejs]] | \[\[node学习node基础/commonJS]] | \[\[node学习node基础/ECMAScript]] node底层使用的语言是libuv，是一个c++语言，他是用来操作底层和操作系统，封装了操作系统接口。Node事件循环是用libuv写的，所有Node的生命周期和浏览器还是有区别的。
...
timers并非真正意义上的队列，他内部存放的是计时器。 每次到达这个队列，见检查计时器线程内的所有计时器，计时器线程内部多个计时器按照时间顺序排列。
...
真正的队列，里边扔的就是待执行的回调函数的集合。类似\[fn,fn]这种形式的。 每次到达check这个队列后，立即按顺序执行回调函数即可 所以说，setImmediate不是一个计时器的概念。
...
以上代码，主线程运行的时候，settimeout函数调用，计时器线程增加一个定时任务。setImmediate函数调用后，其回调立即push到check队列。主线程执行完毕 eventloop判断时，发现timers和check队列有内容，进入异步轮询：
